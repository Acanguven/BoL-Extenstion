...
for(var i=0; i<links.length; i++) {
	for(var x = 0; x < testCases.length; x++){
		if(links[i].href.match(testCases[x].test)){
			links[i].insertAdjacentHTML('beforebegin','<h3 dltype="'+testCases[x].name+'" dllink='+links[i].href+' style="background-color:beige;color:brown;fontSize:30px;cursor:pointer;"><img src="https://raw.githubusercontent.com/thelaw44/BoL-Extenstion/master/arrow_animate.gif">&nbsp;&nbsp;&nbsp;Bot of Legends Helper: Download Script('+decodeURI(links[i].href.replace(/^.*[\\\/]/, ''))+')</h3>');
