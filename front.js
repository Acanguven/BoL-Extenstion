...
for(var i=0; i<links.length; i++) {
	if ((links[i].href.substr((~-links[i].href.lastIndexOf(".") >>> 0) + 2)).toLowerCase().indexOf("lua") == 0){
		links[i].insertAdjacentHTML('beforebegin','<h3 dllink='+links[i].href+' style="background-color:beige;color:brown;fontSize:30px;cursor:pointer;">Bot of Legends Helper: Download Script('+decodeURI(links[i].href.replace(/^.*[\\\/]/, ''))+')</h3>');
